...
- 唐宋食分都是用太半少來表示，《麟德》：「餘半以下，爲半弱；半以上，爲半強。」 0—0.25 爲某度，0.25—0.5 爲某度少或半弱，0.5—0.75 爲某度半，0.75—1 爲某度太或半強。<v>觀天</v>：「其分就近約之爲太、半、少。」少半太是 0.25-0.5-0.75，旣然是就近，那似乎可以理解爲 0—0.125 爲某度，0.125—0.375 爲某度少，0.375—0.625 爲某度半，0.625—0.875 爲某度太，0.875—1 爲某 +1 度。在時間模塊有相應轉換工具。
...
<v>統天</v>發明歲實消長，<v>授時</v>繼之。<v>統天</v>百年減 2.1167 分（滕艳辉、王鹏云、刘娅娅《 统天历的岁实消长与气朔算法分析》），比實際高了 36 倍，這樣三百年下來就有一刻的誤差了。《統天》有歲實消長、朔實消長，《授時》只有歲實消長，按照《授時》術文，加上歲實消長後只有冬至時間會變化，經朔時間毫無變化。《授時》百年減 1 分，歲實大約是百年減 2 分，比統天小了一些，還是比實際高 32 倍，這樣四百年誤差一刻，此後誤差加速增大。

似乎《統天》與《授時》還有個區別，在當年計算中《統天》用的是原本的歲實 .2425，而授時用的消長之後的歲實，這點不太確定。

#### 程序可靠性驗證

元史：《統天》「紹熙二年（1191）辛亥十一月壬申四十七刻」，藤算得 5668.8857 日分 = 0.4724，程序算得壬申 4724，相合。天正定朔藤算得 0157，程序算得 0158，相合。

藤豔輝《宋代朔閏》頁 189 有各年《紀元》、《統天》冬至時刻，算得與藤完全相同。

據刘娅娅《朱载堉对授时历岁余计算的检验与修正》：朱載堉算得隱公三年辛酉（前 720）年前冬至爲庚午 0600，次年冬至乙亥 5044，程序計算與朱載堉相合。
...
先把里差加在通積分上，接著按一般步驟求冬至、冬至日度、五星前合分。求經朔入轉，先用朔積分求得入轉日，再加減里差。「朔積分」是用沒有加減里差的通積分求得的：「置通積分，滿朔實去之，不盡，爲閏餘；以減通積分，爲朔積分」。稍微麻煩一點的是入交，`里差 / 2 / 9` 加到朔積分上，再求入交，為什麼要除 18 呢？按這樣的方法加上里差後，《庚午》比《大明》入轉日多 0.02 日，入交日少 0.002 日，幾乎沒區別。
...
基於夏至前後對稱，以及覆蓋範圍連續性，我覺得應該改爲「自大寒畢立夏」。但後來發現，《五紀》原來是照抄《麟德》：
...
這兩部曆法推算交食都考慮了五星的因素，這是很神奇的思路，當時人為什麼覺得五星對交食有影響呢？值得探討一下。也許因為星和日月有牽制作用？
...
729 年主要區別是在定朔上，《中》是 3402，程序算得 3457，程序的月亮改正大了一些。大衍曆應該沒什麼問題了。
...
另外，藤豔輝《宋代朔閏》也有崇天紀元的計算結果，但是跟後來發的論文出入很大，估計是這篇論文的程序還有問題。
...
除了三個不太相合的，《紀元》基本上沒問題。應該是特殊情況的處理沒對，留待以後考慮，也不一定，萬一是藤豔輝錯了呢是吧。
...
10 沒說按照哪部曆法算出來八分，如果按照我的結果，就是《開禧》，按照藤的結果，是《統天》。如果單看這一條，我的程序應該比藤準確。統天差得有點多，估計是歲實消長算法不一樣。我紀元的月食時差似乎有點問題。

#### 授時

《中國古代曆法》頁 669 有三次日食推算：

| 日期      | 元史食分 | 《中》食分 | 程序食分 | 《中》食甚 | 程序食甚 | 備註                    |
| --------- | -------- | ---------- | -------- | ---------- | -------- | ----------------------- |
| 1292 正月 |          | 7.168      | 9.48     | 5481.8     | 5477     | 《中》交常度似乎少了2度 |
| 1299 八月 | 二分有奇 | 2.67       | 2.72     | 4390       | 4389     |                         |
| 1302 六月 | 五十七秒 | 1.15       | 0.59     | 8058       | 8062     |                         |

《授時》交食應該問題不大，程序算出五十九秒，與史載五十七秒幾乎不差，勝過《中》。

李勇《授時曆與仲康日食推算》列出了前 2200—前 1799 年《授時曆日食典》，《授時曆預報2012年日食》列出了 1990—2050 日食典。經過核對，發現連食甚時刻都差了許多，因此食分也差了 1 分左右，最大的可能是歲實消長算法不一樣。
