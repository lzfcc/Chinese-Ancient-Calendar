...
交點退行速度用此公式計算： `Sidereal = (MoonAvgV + v退) * Node`《明天》步交會中的「朔差」表示一個朔望月交點退行距離。（藤豔輝《宋代朔閏》頁 95）。再把月平行速 `MoonAvgV = Sidereal/Lunar + 1` 公式代入，得 `v退 = Sidereal/Node - Sidereal/Lunar - 1`

#### 交率交數

計算入交定日的公式：`入交定日 = 入交泛日 + 日改正 ± 交率/交數 * 月改正`

恆星月日數/恆星年日數= s/m ，`交率/交數 = 交點月/交點年 = (恆星月-v退) / (恆星年-v退) = 27.2122/346.608 ≒ 0.0785` （見劉金沂《隋唐曆法中入交定日術的幾何解釋》，《自然科學史研究》1983(4)）

#### 食限

《宣明》、《應天》、《乾元》、《儀天》陽曆食限 4.2 度左右，陰曆 9.6 度，《崇天》、《觀天》陽 5.3，陰 8.84，《紀元》陽 4.88，陰 9.76。
...
【欽天乾元儀天崇天觀天庚午】朔入交定日 = 朔入交常日 ± 比例 × 月改正（乾元：度定分）
...
【庚午】庚午和紀元幾乎全同，除了這條：食甚泛餘 = 經朔 ± 日月改正(朔入氣入轉) * 1337 / 轉定分(定朔入轉)

不論陰陽，近交初為交初，在二十六日、二十七日為交初；近交中為交中，在十三日、十四日為交中。交初交中主要用來判斷食差符號。

【崇天觀天】月入陰陽曆積度 = 入陰陽曆 * 月平行速

【紀元庚午】定朔入交泛日 = 經朔入交 ± 日月改正

【紀元庚午】定朔入交積度 = 定朔入交泛日 * 月平行速。定朔入交定積度 = 定朔月行入交積度 ± 月改正(定朔入轉)。

#### 三、時差

只有紀元、授時有月食時差，其他都以定望爲食甚。

【欽天】距午分(定朔)【應天乾元儀天崇天】午前後分(定朔) 

【應天乾元儀天】時差(午前後分, 半晝分) 

【崇天】時差(午前後分) 【欽天觀天】時差(定朔)
