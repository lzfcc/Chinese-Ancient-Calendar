...
另外，「四七日」指的是近點月中 6.8886、13.7772、20.6656、27.5545 這四個七日（向上取整），這四點是轉折點，當日損益率需要分段處理。
...
氣差（應天、儀天：赤道差，乾元：離差，明天：南北差）

注：在敘述的時候應天先黃道差後赤道差，所以曲安京、藤豔輝都認為黃道差是氣差，赤道差是刻差，但是從符號判定方法以及複雜程度來看，應該反過來纔是。
...
【應天乾元儀天明天】食差 = 氣差 ± 刻差（明天：食差總數） 【崇天觀天】食差 = 氣差 ± 刻差 ± 時差【紀元】 食差 = 氣差 ± 刻差 ± k0
...
【紀元】朔入交定日 = 入交常日 ± 食差。望入交定日 = 入交常日 ± 比例*月改正(望入轉)（卽乾元儀天的入交定日）
