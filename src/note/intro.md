...
- 增加天文模塊的黃赤宿鈐。改寫「積度 ⇌ 宿度」。
- 合併`eclpGong2Mansion` 和 `equaGong2Mansion`，改寫赤道宿度算法（待定）。
...
- 紀元曆書1598差了半個月。1598正月沒有立春，在去年。庚午總是多了60天
- 日月食計算應該還有一些小問題：<v>應天</v>怪怪的。
- <v>劉孝孫</v>入交不對。<v>觀天</v>入交不對，重新擬了一個交點月，暫時解決，以後再研究。
