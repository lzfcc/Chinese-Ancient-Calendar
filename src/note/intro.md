...
明天精度很成問題，和紀元、統元比，入轉差了 3 日，入交差了 1.6 日。明天上元以陰曆入交。

#### 5-20 晚 `0.991.2`

**核心** 修復定朔定氣等等缺省的情況，主要針對太乙曆。cal目錄的package.json合併到根目錄。根據一段材料重新構擬占天。增補交食參數。觀天入交很奇怪，暫時重新擬一個 node。合併 NodeCorr、NodeOrigin，AnomaCorr、AnomaOrigin。酌情改幾部南宋曆的上元宿度，改統天的宿度。
