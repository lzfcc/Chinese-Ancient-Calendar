...
6. 本工具完全開源，如您需要査詢、核査各曆的詳細參數與算法，請前往源碼倉庫。如您不嫌麻煩，可以下載源碼，在本地運行程序。更多技術特徵見開發者說明。核心計算邏輯文件大小：252KB。 **隱私聲明** 本站使用了 Google Analytics 進行流量分析；評論系統採用 Hyvor Talk；服務器託管在 Netlify 平臺。運算均在本地瀏覽器進行。
6. 可以參考《律曆初階下》：
...
4-21 `1.14` 增加清華簡《筮法》構擬。周易筮法算法調整為通行算法。前端 `1.07` 更新依賴：node 18.16.0
