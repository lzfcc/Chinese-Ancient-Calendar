...
function PlusFunctionMinusT(offset, PrevThisNext, CommonPara) {
    const FirstSyzygyThis = {}
    const FirstSyzygyPrev = {}
    const FirstSyzygyNext = {}
    FirstSyzygyThis.First = FirstSyzygy(1, 0, CommonPara)
    if (offset == 0) {
        FirstSyzygyThis.Syzygy = FirstSyzygy(0, 0, CommonPara)
    }
    if (PrevThisNext == -1) {
        FirstSyzygyPrev.First = FirstSyzygy(1, -1, CommonPara)
    }
    if (PrevThisNext == 1) {
        FirstSyzygyNext.First = FirstSyzygy(1, 1, CommonPara)
    }
    return {
        FirstSyzygyThis,
        FirstSyzygyPrev,
...
function FirstSyzygy(offset, PrevThisNext, CommonPara) {
    const {
        year,
        SolarRaw,
        FirstMonNum,
    ...
    const AccumZhong = (OriginYear + PrevThisNext) * solar
    ...
    for (let i = 1; i <= 13; i++) {
        ...
        AvgRaw[i] = AccumTong - LeapSurAvg + (i - (offset ? 1 : 0.5) + FirstMonNum) * lunar // 經朔望
        const WsolsticeAccum = ((i - (offset ? 1 : 0.5) + FirstMonNum) * lunar - LeapSurAvg + solar) % solar // 各月朔入冬至的積日
        ...
        AnomaAccum[i] = ((AnomaMonConst + AccumZhong - LeapSurAvg + (i - (offset ? 1 : 0.5) + FirstMonNum) * lunar) % AnomaLunar + AnomaLunar) % AnomaLunar // 入轉日分
        let sign = 1
        let FaSlowT = 0
...
function CalShoushi(ShouPre, year) {
    ...
    const This = PlusFunctionMinusT(1, 0,CommonPara)
    const Prev = PlusFunctionMinusT(1, -1,CommonPara)
    const Next = PlusFunctionMinusT(1, 1,CommonPara)
    const AccumTongThis=This.AccumTong
    const LeapSurAvgThis = This.LeapSurAvg
    const ifLeapAvgThis = LeapSurAvgThis >= LeapStd ? 1 : 0
    const LeapSurAvgPrev = Prev.LeapSurAvg
    const ifLeapAvgPrev = LeapSurAvgPrev >= LeapStd ? 1 : 0
    const LeapSurAvgNext = Next.LeapSurAvg
    const ifLeapAvgNext = LeapSurAvgNext >= LeapStd ? 1 : 0
    const LeapSurAcrThis = This.LeapSurAcr
    const ifLeapAcrThis = LeapSurAcrThis >= LeapStd ? 1 : 0 // 是否有閏月
    const LeapNumAcrThis = ifLeapAcrThis > 0 ? Math.ceil((lunar - LeapSurAcrThis) / MonLeap) : 0 // 閏月數
    const LeapNumTerm = LeapNumAcrThis
    const LeapSurAcrPrev = Prev.LeapSurAcr
    const ifLeapAcrPrev = LeapSurAcrPrev >= LeapStd ? 1 : 0
    const LeapSurAcrNext = Next.LeapSurAcr
    ...
    const TermSC = []
    const TermDecimal = []
    const FirstOrderRaw = This.OrderRaw
    // 中氣
    for (let i = 1; i <= 12; i++) {
