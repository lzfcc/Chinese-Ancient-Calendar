...
export const Equator2EclipticFormula = (LongiRaw, CalName) => { // 公式化的，週天度就用自己的
    ...
    let EclipticLongi = 0
    let EquatorLongi = 0
    let Longi = LongiRaw % (Sidereal / 8)
    if ((LongiRaw > Sidereal / 8 && LongiRaw <= Sidereal / 4) || (LongiRaw > 3 * Sidereal / 8 && LongiRaw <= Sidereal / 2) || (LongiRaw > 5 * Sidereal / 8 && LongiRaw <= 3 * Sidereal / 4) || (LongiRaw > 7 * Sidereal / 8)) {
        Longi = Sidereal / 8 - Longi
    }
    let LongiDif = 0
    ...
    // 《古代曆法》頁123    沒明白。曲安京《中国古代的二次求根公式与反函数》，西北大学学报(自然科学版). 1997(01)。曆法中二次反函數僅有的例子是大衍行星行度、紀元。赤道度爲Sidereal/8，黃道度就是43.1287。兩篇公式不一樣，最後畫圖才想明白。
    let h = 0
    if (CalName === 'Jiyuan') {
        // if (LongiRaw < Sidereal / 4 || (LongiRaw >= Sidereal / 2 && LongiRaw < Sidereal * 0.75)) {
...
export const Hushigeyuan = (LongiRaw, Sidereal) => {
    ...
    let sign = 1
    if (LongiRaw < Sidereal / 4 || LongiRaw > Sidereal * 0.75) {
        Lati = -Lati
        sign = -1
    }
    const Lati1 = Sidereal / 4 - Lati
    //////////晷漏//////// 北京緯度40.95
...
export const Hushigeyuan2 = LongiRaw => {
    ...
    const a0 = k * v0 / a
    let EquatorLongi = 0
    if (LongiRaw < Sidereal / 2) {
        EquatorLongi = Sidereal / 4 + a0
    } else {
        EquatorLongi = Sidereal / 4 - a0
    }
    return EquatorLongi
...
export const Longi2LatiFormula = (LongiRaw, CalName) => { // 《中國古代曆法》頁128。漏刻頁135
    ...
    if (CalName === 'Yitian') {
        LongiRaw -= SunFormula1(LongiRaw, CalName)
    }
    let Longi = LongiRaw % (Sidereal / 2)
    let Longi1 = LongiRaw % (Sidereal / 2)
    if (Longi > Sidereal / 4) {
        Longi = Sidereal / 2 - Longi
    }
    let Lati = 0
    let g = 0
    if (CalName === 'Chongxuan') { // x=195.838,y=0. x=138.478,y=35.267極值。x=91.3, y=23.996
        ...
        if (LongiRaw >= Sidereal / 4 && LongiRaw < 3 * Sidereal / 4) {
            Lati = 23.9141 - g
        } else {
            Lati = -23.8859 + g
        }
        ...
        if (LongiRaw >= Sidereal / 4 && LongiRaw < 3 * Sidereal / 4) { // 冬至後次象
            if (Longi1 > 93.7412) {
                Longi1 = Sidereal / 2 - Longi1
            }
        ...
        if (LongiRaw >= Sidereal / 4 && LongiRaw < 3 * Sidereal / 4) { // 夏至前後
...
// console.log(Longi2LatiFormula(100, 'Yitian').Lati1)
...
export const Longi2DialFormula = (DegRaw, CalName) => { // 崇玄的Day沿用大衍：正午與二至時刻的距離加上日躔。陈美東《崇玄儀天崇天三曆晷長計算法及三次差內插法的應用》。1、距二至的整數日，2、算上二至中前後分的修正值。我現在直接用正午到二至的距離。之所以那麼麻煩，應該是因為整數好算一些，實在迷惑。   // ：冬至到夏至，盈縮改正爲負，入盈曆，實行日小於平行日。因此自變量不應該是黃經，而是！！！！達到實行度所需日數！！！！！崇玄、崇天爲日躔表的盈縮分，儀天爲公式先後數，也就是定朔計算中的SunTcorr，只是符號相反。崇玄、崇天的節接銜接不理想。
    ...
    let Deg = parseFloat((DegRaw % (Solar / 2)).toPrecision(14))

    let Dial = 0
    if (CalName === 'Chongxuan') {
        if ((DegRaw >= Solar / 2 && Deg > 123.62225) || (DegRaw < Solar / 2 && Deg > 59)) {
            Deg = parseFloat((Solar / 2 - Deg).toPrecision(14))
        }
        if (DegRaw <= 59 || (DegRaw >= 123.62225 + Solar / 2)) {
        ...
        if (DegRaw <= 59 || (DegRaw >= 123.622275 + Solar / 2)) {
            Dial = 12.715 - 1e-6 * (2130 - 14 * Deg) * Deg ** 2
        } else {
            Dial = 1.478 + 1e-7 * (4812 - 3.5 * Deg) * Deg ** 2
        }
    } else if (CalName === 'Chongtian') {
        if ((DegRaw >= Solar / 2 && Deg > 120.62) || (DegRaw < Solar / 2 && Deg > 62)) {
            Deg = parseFloat((Solar / 2 - Deg).toPrecision(14))
        }
        if (DegRaw <= 62 || (DegRaw >= 120.62 + Solar / 2)) {
            Dial = 12.715 - 1e-6 * (2197.14 - 15.05 * Deg) * Deg ** 2
        } else {
            Dial = 1.478 + 1e-7 * (4881.67 - 4.01 * Deg) * Deg ** 2
        }
    } else if (CalName === 'Mingtian' || CalName === 'Guantian') {
        if ((DegRaw >= Solar / 2 && Deg > 137) || (DegRaw < Solar / 2 && Deg > 45.62)) {
            Deg = parseFloat((Solar / 2 - Deg).toPrecision(14))
        }
        Deg = big(Deg)
        if (DegRaw <= 45.62 || (DegRaw >= 137 + Solar / 2)) {
        ...
        } else if (DegRaw > Solar / 4 && DegRaw < 3 * Solar / 4) {
        ...
        if ((DegRaw >= Solar / 2 && Deg > 120.42) || (DegRaw < Solar / 2 && Deg > 62.2)) {
            Deg = parseFloat((Solar / 2 - Deg).toPrecision(14))
        }
        Deg = big(Deg)
        if (DegRaw <= 62.2 || (DegRaw >= 120.42 + Solar / 2)) {
...
export const MoonLongiFormula = (OriginRawRaw, Day, CalName) => { // 該日距冬至黃道度，入交日。不知是否應該加上日躔
    ...
    let Origin = OriginRaw % (Solar / 2)
    if (Origin > Solar / 4) { // 這一步沒有說明
        Origin = Solar / 2 - Origin
    }
    if (Longi > Xiang / 2) {
        Longi = Xiang - Longi
    }
    ...
    if (CalName === 'Shoushi') {
        ...
        if (CalName === 'Chongtian') {
            ...
            if (LongiRaw <= Node / 2 && OriginRaw <= Solar / 2) {
                const F5 = Math.abs(Origin - Solar / 4)
                EquatorWhiteDif = 9 * F5 * Longi * (101 - Longi) / 16000 * Xiang
            } else if (LongiRaw > Node / 2 && OriginRaw > Solar / 2) {
                const F6 = Math.abs(Origin - 3 * Solar / 4)
