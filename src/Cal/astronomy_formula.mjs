...
export const Equa2EclpFormula = (LongiRaw, Name) => { // 公式化的，週天度就用自己的
...
export const Longi2LatiFormula = (LongiRaw, Name) => { // 《中國古代曆法》頁128。漏刻頁135
    const Solar = AutoSidereal(Name)
    const Solar25 = Solar / 4
    const Solar50 = Solar / 2
...
export const Longi2DialFormula = (DegRaw, Name) => { // 崇玄的NodeAccum沿用大衍：正午與二至時刻的距離加上日躔。陈美東《崇玄儀天崇天三曆晷長計算法及三次差內插法的應用》。1、距二至的整數日，2、算上二至中前後分的修正值。我現在直接用正午到二至的距離。之所以那麼麻煩，應該是因爲整數好算一些，實在迷惑。   // ：冬至到夏至，盈縮改正爲負，入盈曆，實行日小於平行日。因此自變量不應該是黃經，而是！！！！達到實行度所需日數！！！！！崇玄、崇天爲日躔表的盈縮分，儀天爲公式先後數，也就是定朔計算中的SunTcorr，只是符號相反。崇玄、崇天的節接銜接不理想。
...
export const MoonLongiFormula = (NodeEclpLongi, MoonNodeDifRev, Name) => { // SunEclpLongi, NodeAccum,  // 該日距冬至黃道度，入交日。不知是否應該加上日躔
    const Solar = AutoSolar(Name)
    ...
    // }
    // 《數》頁359
    let EclpWhiteDif = Math.abs(AutoEqua2Eclp(MoonNodeDifRev, Name).Equa2EclpDif) / 2 // AutoEqua2Eclp(MoonEclpLongiRev, Name)
    if (Name === 'Jiyuan') {
