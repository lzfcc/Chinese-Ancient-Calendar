...
const GenAcrV = () => { // 生成殘曆的躔離
    ...
    for (let i = 0; i <= 28; i++) {
        MoonAcrDV[i] = Math.round((MoonAcrVList1[i] + MoonAcrVList2[i]) / 2)
        // MoonAcrDV[i] = Math.round(10000 * MoonAcrVList1[i] / Denom1)
    }
    for (let i = 0; i <= 27; i++) {
...
// 應天日食食分算法廢：
// let tmp = (0.75 - TotalDeci) * MoonAvgDV * 20
// if (TotalDeci > 0.5) {
//     tmp /= 4
...
// MoonAcrDV = (MoonAcrVListA[AnomaAccumPart] + PartRange / MoonAcrVListB[AnomaAccumPart]) / 2
...
// }
// MoonAcrDV = 1.0963 + signB * (0.11081575 - 0.0005815 * AnomaXian - 0.00000975 * AnomaXian * (AnomaXian - 1)) // 限下行度