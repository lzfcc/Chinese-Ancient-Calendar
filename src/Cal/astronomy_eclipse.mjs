...
const Eclipse1 = (NodeAccum, CalName) => {
    ...
    const Limit = ['Daming', 'Kaihuang'].includes(CalName) ? MoonAvgDV * (Lunar - Node) / 2 : Lunar / 2
...
const EcliTcorr3 = (isNewm, isYin, CalName, Type, Denom, Solar25, Solar75, NewmNoonDif, NewmNoonDifAbs, Rise, RiseNoonDif, AvgTotalDeci, AvgTotalNoonDif, AcrDeci, AvgDeci, AvgMoonTcorr, MoonAcrDV, SunTcorr, NodeDif, AcrSolsDif) => {
    let Tcorr = 0
        ...
        if (['Dayan', 'Wuji', 'Tsrengyuan', 'Xuanming'].includes(CalName)) {
            ...
            Last = Math.sqrt(((isNewm ? 20 : 30) - Magni) * Magni)
                * (['Datong', 'Datong2'].includes(CalName) && !isNewm ? 0.00492 : 0.00574)
                / (MoonFormula(AcrAnomaAccum, CalName).MoonAcrDV - 0.082)
        }
        if (['Wuji', 'Tsrengyuan'].includes(CalName)) {
            let Portion = 0.5
            if (isNewm) {
...
const Eclipse3 = (AvgNodeAccum, AvgAnomaAccum, AcrDeci, AvgDeci, AcrSolsDif, AvgSolsDif, Rise, SolsAccum, isNewm, CalName) => {
    ...
    const { Tcorr, TotalDeci, TheTotalNoonDif, dd, isSame } = EcliTcorr3(isNewm, isYin, CalName, Type, Denom, Solar25, Solar75, NewmNoonDif, NewmNoonDifAbs, Rise, RiseNoonDif, AvgTotalDeci, AvgTotalNoonDif, AcrDeci, AvgDeci, AvgMoonTcorr, MoonAcrDV, SunTcorr, NodeDif, AcrSolsDif)
    ////////////////////// 食差
