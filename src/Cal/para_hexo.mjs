...
const ListNVIIA = [4004, 4005, 4006, 8014, 8015, 8016]; //21111
const ListNVIIB = [14022, 15023, 15024, 18028, 18029]; //21112
const ListNVIIC = [19030, 19031, 20032, 20033]; // 21223
const ListNVII = [
...
const ListNVIII = [7011, 16025, 16026, 17027]; // 11110
const ListNVandVIII = [
  22035, 23039, 23040, 24042, 24043, 24044, 24045, 26046, 36056, 42062, 44064,
...
const ListNIXB = [52073, 52074, 52075, 52076, 52077, 52078, 52079]; // 11021
const ListNIXS = [
  52074, 52075, 52078, 57089, 57093, 57096, 57097, 58099, 58101, 61110,
];
const ListNX = [53080, 53081, 53082, 54083, 54084]; // 21020
const ListNXI = [
...
const ListNXII = [7011, 10018, 16025, 17027, 22035, 24042, 24043, 39059, 50071]; // 10010, true
const ListNXIII = [
  4004, 4005, 5007, 6009, 6010, 7012, 9017, 13021, 18028, 20033, 23038, 27047,
  31051, 35055, 41061, 43063,
]; // 20010, true
const ListNThree = [
...
const ListPI = [0, 5.8824, 24.8366, 49.0196, 18.3007, 1.9608, 306]; // 几个分系的实际概率+爻数
const ListPII = [0, 5.8824, 43.5294, 35.6863, 13.7255, 1.1765, 255];
const ListPIII = [0, 2.9762, 44.6429, 40.4762, 10.119, 1.7857, 168];
const ListPIV = [0, 4.6296, 38.8889, 37.963, 15.7407, 2.7778, 108];
const ListPV = [0, 4.5455, 36.3636, 48.4848, 10.6061, 0, 66];
const ListPVIandVII = [0, 4.712, 45.5497, 36.6492, 12.0419, 1.0471, 191];
const ListPVandVIII = [0, 7.7778, 36.6667, 47.7778, 7.7778, 7, 90];
const ListPIXB = [0, 7.1429, 30.9524, 33.3333, 23.8095, 4.7619, 42];
const ListPXI = [0, 8.3333, 35.6061, 38.6364, 15.9091, 1.5152, 132];
...
export const ListPAll = [
  ...
  [0, 16.25, 31.25, 33.75, 15.0, 3.75, 80],
];

// 分系
const Devide = (list, screen, isStrict) => {
  let result = "";
  for (const [key, value] of Object.entries(list)) {
    if (isStrict) {
      ...
      for (let j = 0; j < value[1].length; j++) {
        if (value[1][j] === "4" || value[1][j] === "5" || value[1][j] === "9") {
          l++;
        }
      }
      for (let i = 0; i < 5; i++) {
...
// console.log(Devide(HexoZhouList, "11020", true));
// 各分系的概率
...
// console.log(Count(HexoZhouList, [ListNIXS]));
