...
export const BindTcorr = (AnomaAccum, WinsolsDifRaw, year, CalName) => {
    ...
    let List1 = ['Qianxiang', 'Jingchu', 'Yuanjia', 'Daming', 'Tsrengguang', 'Xinghe', 'Tianbao', 'Daye', 'WuyinA', 'Huangji', 'LindeA', 'Wuji', 'Tsrengyuan', 'Futian', 'Qintian', 'Mingtian', 'Jiyuan', 'Tongyuan', 'Qiandao', 'Chunxi', 'Daming3', 'Huiyuan', 'Tongtian', 'Kaixi', 'Chunyou', 'Huitian', 'Chengtian', 'ShoushiB']
    ...
    Print1 = Print1.concat(
        List1.map(title => {
            ...
            const { SunTcorr, MoonTcorr, NodeTcorr } = AutoTcorr(AnomaAccum, WinsolsDifRaw, title)
            ...
            if (NodeTcorr) {
                NodeAccumCorrPrint = NodeTcorr.toFixed(4)
            }
    ...
    Print2 = Print2.concat(
        List2.map(title => {
            ...
            const { SunTcorr, MoonTcorr, NodeTcorr
            ...
            return {
                title: CalNameList[title],
                data: [SunDifAccumPrint, SunDifAccumInacPrint, MoonAcrSPrint, MoonDifAccumPrint, MoonDifAccumInacPrint, SunTcorrPrint, SunTcorrInacPrint, MoonTcorrPrint, MoonTcorrInacPrint, Tcorr.toFixed(4), NodeTcorr.toFixed(4)]
            }
        }))
...
export const BindEqua2Eclp = (LongiRaw, Sidereal, year) => {
    ...
    const List1 = ['Qianxiang', 'Huangji', 'Dayan', 'Chongxuan', 'Qintian', 'Yingtian', 'Qianyuan', 'Yitian', 'Chongtian', 'Mingtian', 'Guantian', 'Jiyuan', 'ShoushiB']
    const List2 = ['Chongxuan', 'Yitian', 'Chongtian', 'Mingtian', 'Guantian', 'Jiyuan', 'ShoushiB']
    Print = Print.concat(
        List1.map(title => {
            let EclpLongiPrint = '-'
...
export const BindDeg2Mansion = (Deg, CalName) => {
    ...
    const Print = ['Taichu', 'Huangji', 'LindeA', 'Dayan', 'Yingtian', 'Mingtian', 'Jiyuan', 'Daming3', 'ShoushiB'].map(title => {
...
export const BindLongi2Lati = (LongiRaw, WinsolsDeci, f, Sidereal, year) => {
    ...
    Print = Print.concat(
        ['Easthan', 'Yuanjia', 'Daming', 'Daye', 'WuyinA', 'Huangji', 'LindeA', 'Dayan', 'Xuanming', 'Chongxuan', 'Yingtian', 'Qianyuan', 'Yitian', 'Chongtian', 'Mingtian', 'Guantian', 'Jiyuan', 'Daming3', 'ShoushiB'].map(title => {
            let Lati1Print = '-'
            let LatiPrint = '-'
            let LatiInacPrint = '-'
            let SunrisePrint = '-'
...
export const BindMoonLongiLati = (NodeAccum, MoonEclp) => { // 該時刻入交日、距冬至日數
    ...
    let Print = []
    Print = Print.concat(
        ['Qianxiang', 'Yuanjia', 'Daming', 'Huangji', 'Dayan', 'Wuji', 'Tsrengyuan', 'Chongxuan', 'Qintian', 'Yingtian', 'Chongtian', 'Mingtian', 'Guantian', 'Jiyuan', 'ShoushiB'].map(title => {
...
export const BindSunEclipse = (NodeAccum, AnomaAccum, AvgDeci, AvgWinsolsDif) => {
    ...
    let Print1 = []
    Print1 = Print1.concat(
        ['Daye', 'WuyinA', 'Huangji', 'LindeA', 'Wuji', 'Tsrengyuan', 'Qintian', 'Jiyuan', 'Tongyuan', 'Qiandao', 'Chunxi', 'Huiyuan', 'Tongtian', 'Kaixi', 'Chengtian', 'Daming3', 'Gengwu', 'ShoushiB', 'Datong'].map(title => {
...
export const BindMoonEclipse = (NodeAccum, AnomaAccum, AvgDeci, AvgWinsolsDif) => {
    ...
    let Print1 = []
    Print1 = Print1.concat(
        ['Tsrengguang', 'Daye', 'WuyinA', 'Huangji', 'LindeA', 'Wuji', 'Tsrengyuan', 'Qintian', 'Jiyuan', 'Tongyuan', 'Qiandao', 'Chunxi', 'Huiyuan', 'Tongtian', 'Kaixi', 'Chengtian', 'Daming3', 'Gengwu', 'ShoushiB', 'Datong'].map(title => {
            const { Tcorr1, Tcorr2 } = AutoTcorr(AnomaAccum, AvgWinsolsDif, title)
            const AcrDeci = (AvgDeci + (Tcorr2 || Tcorr1) + 1) % 1
            const AcrWinsolsDif = AvgWinsolsDif + (Tcorr2 || Tcorr1)
            const { Magni, StartDeci, TotalDeci, EndDeci, status
...
// console.log (InacPrintAnaly_SunTcorr('ShoushiB', 7, 1247))